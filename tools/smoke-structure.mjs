...
const checks = [
  ...
  ['legacy hair profiles shrink toward scalp', /widthScale:\s*0\.80/.test(attachmentFit) && /widthScale:\s*0\.86/.test(attachmentFit) && /yOffsetMul:\s*-0\.12/.test(attachmentFit)],
  ['legacy hair profiles seat toward hairline', /zOffsetMul:\s*0\.045/.test(attachmentFit) && /zOffsetMul:\s*0\.035/.test(attachmentFit)],
  ...
  ['rear chain radius is final-fit tightened', /backWidthScale:\s*0\.30/.test(attachmentFit) && /backForward:\s*0\.050/.test(attachmentFit) && /backLift:\s*0\.002/.test(attachmentFit)],
