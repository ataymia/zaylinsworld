...
const checks = [
  ...
  ['dense head cage contract', /HEAD_CAGE_POINTS/.test(attachmentFit) && /HEAD_LANDMARK_NAMES/.test(attachmentFit) && /crownFront/.test(attachmentFit) && /napeCenter/.test(attachmentFit)],
  ...
  ['hair uses dense per-vertex cage', /warpPointToCage/.test(attachments) && /dense-head-cage-v2/.test(attachments) && /HEAD_CAGE_POINTS/.test(attachments)],
  ['hair height is capped per style', /maxHeightMul/.test(attachmentFit) && /Math\.min\(naturalHeight/.test(attachments)],
  ...
  ['neck loop surface contract', /NECK_RING_SEGMENTS/.test(attachmentFit) && /necklaceSurfacePath/.test(attachments) && /neckEnvelope/.test(attachments)],
  ['chain projects onto visible surface', /weightedSurfaceZ/.test(attachments) && /projectedChestDrape/.test(attachments) && /surfaceSamples/.test(attachments)],
  ...
  ['failed jewelry mount can retry', /surface jewelry mount failed/.test(attachments) && /instance\.jewelryKey = null/.test(attachments)],
