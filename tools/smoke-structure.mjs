...
const checks = [
  ...
  ['native scalp references exist', /NATIVE_SCALP_REFERENCE_NODES/.test(attachmentFit) && /ZW_Hair_CloseCrop/.test(attachmentFit) && /ZW_Hair_CrewCut/.test(attachmentFit)],
  ...
  ['hair samples pack-native fitted meshes', /function nativeScalpField/.test(attachments) && /NATIVE_SCALP_REFERENCE_NODES/.test(attachments) && /vertexInInstanceSpace/.test(attachments)],
  ['hair binds roots to curved scalp surface', /function sourceHairField/.test(attachments) && /function queryScalpPoint/.test(attachments) && /function rootAdhesion/.test(attachments) && /native-pack-scalp-surface-v3/.test(attachments)],
  ['hair preserves style volume away from roots', /sourceShellRadius/.test(attachments) && /volumeScale/.test(attachments) && /releaseBelow/.test(attachments)],
  ...
  ['chain is one closed collarbone loop', /function necklaceLoop/.test(attachments) && /new THREE\.CatmullRomCurve3\(localPoints, true/.test(attachments) && /single-closed-collarbone-loop-v3/.test(attachments)],
  ['chain projects onto front and back surfaces', /weightedSurfaceZ\(metrics, x, y, true\)/.test(attachments) && /weightedSurfaceZ\(metrics, x, y, false\)/.test(attachments)],
  ['chain uses individual links', /ZW_ChainLink_/.test(attachments) && /TorusGeometry/.test(attachments) && !/TubeGeometry/.test(attachments)],
  ['pendant is connected at front center', /const frontWorld = worldPoints\[0\]/.test(attachments) && /ZW_PendantBail_/.test(attachments) && /pendantClearance/.test(attachments)],
  ...
  ['failed jewelry mount can retry', /closed loop jewelry mount failed/.test(attachments) && /instance\.jewelryKey = null/.test(attachments)],
