...
const checks = [
  ...
  ['hair seats down and back onto scalp', /yOffsetMul:\s*0\.16/.test(attachmentFit) && /yOffsetMul:\s*0\.10/.test(attachmentFit) && /zOffsetMul:\s*-0\.006/.test(attachmentFit)],
  ...
  ['chain uses neck-sized uniform side and rear ellipse', /neckWidthMul:\s*0\.30/.test(attachmentFit) && /shoulderWidthMul:\s*0\.13/.test(attachmentFit) && /backWidthScale:\s*1\.00/.test(attachmentFit)],
  ['chain preserves a controlled front drape', /frontWidthBoost:\s*0\.18/.test(attachmentFit) && /drop:\s*0\.072/.test(attachmentFit) && /frontDropPower:\s*1\.55/.test(attachmentFit)],
