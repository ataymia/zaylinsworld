...
const checks = [
  ...
  ['hair contracts inward instead of only shifting', /widthScale:\s*0\.66/.test(attachmentFit) && /widthScale:\s*0\.74/.test(attachmentFit) && /heightScale:\s*1\.00/.test(attachmentFit)],
  ['hair crown seating offsets are positive', /yOffsetMul:\s*0\.20/.test(attachmentFit) && /yOffsetMul:\s*0\.14/.test(attachmentFit)],
  ...
  ['rear chain contracts without changing front drape', /backWidthScale:\s*0\.20/.test(attachmentFit) && /backForward:\s*-0\.018/.test(attachmentFit) && /frontWidthBoost:\s*0\.06/.test(attachmentFit)],
