...
const checks = [
  ...
  ['hair uses final downward seat', /yOffsetMul:\s*0\.095/.test(attachmentFit) && /yOffsetMul:\s*0\.085/.test(attachmentFit) && /yOffsetMul:\s*0\.055/.test(attachmentFit) && /yOffsetMul:\s*0\.035/.test(attachmentFit) && /zOffsetMul:\s*-0\.010/.test(attachmentFit)],
  ...
  ['whole-body attachment blueprint exists', /AVATAR_ATTACHMENT_BLUEPRINT/.test(attachmentBlueprint) && /AVATAR_BODY_SURFACE_MAP/.test(attachmentBlueprint) && /preserveAuthoredShape/.test(attachmentBlueprint)],
  ...
  ['whole-body blueprint maps named surface regions', ['scalp', 'face', 'neck', 'upperTorso', 'lowerTorso', 'hips', 'leftArm', 'rightArm', 'leftFoot', 'rightFoot'].every((region) => attachmentBlueprint.includes(`${region}: region`))],
  ...
  ['arm solver rejects unrelated prop anchors', /function armEndpoint/.test(avatarSkin) && /isDescendantOf\(explicitEnd, upperArm\)/.test(avatarSkin)],
  ['player hands-down waist pose', /zwRelaxedArmPose/.test(avatarSkin) && /waistPose:\s*true/.test(avatarSkin) && /new THREE\.Vector3\(0\.14, -1, 0\.03\)/.test(avatarSkin) && /new THREE\.Vector3\(-0\.14, -1, 0\.03\)/.test(avatarSkin)],
  ['NPC hands-down waist pose', /relaxedArms/.test(avatarSkin) && /new THREE\.Vector3\(0\.12, -1, 0\.03\)/.test(avatarSkin) && /new THREE\.Vector3\(-0\.12, -1, 0\.03\)/.test(avatarSkin)],
