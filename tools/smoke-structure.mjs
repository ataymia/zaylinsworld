...
const checks = [
  ...
  ['hair receives final downward micro-seat', /yOffsetMul:\s*0\.125/.test(attachmentFit) && /yOffsetMul:\s*0\.065/.test(attachmentFit) && /yOffsetMul:\s*0\.085/.test(attachmentFit) && /zOffsetMul:\s*-0\.006/.test(attachmentFit)],
  ...
  ['rear chain contracts hard toward nape', /backWidthScale:\s*0\.42/.test(attachmentFit) && /backForward:\s*0\.095/.test(attachmentFit) && /backTighten/.test(attachments)],
