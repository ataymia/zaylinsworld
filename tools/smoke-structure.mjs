...
const checks = [
  ...
  ['hair receives only final downward micro-seat', /yOffsetMul:\s*0\.14/.test(attachmentFit) && /yOffsetMul:\s*0\.08/.test(attachmentFit) && /yOffsetMul:\s*0\.10/.test(attachmentFit) && /zOffsetMul:\s*-0\.006/.test(attachmentFit)],
  ...
  ['rear chain contracts toward nape independently', /backWidthScale:\s*0\.62/.test(attachmentFit) && /backForward:\s*0\.038/.test(attachmentFit) && /backTighten/.test(attachments)],
