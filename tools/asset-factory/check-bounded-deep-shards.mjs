...
const gapSummary = existsSync(GAP_SUMMARY_PATH) ? JSON.parse(readFileSync(GAP_SUMMARY_PATH, 'utf8')) : null;
...
function slug(value) {
  return String(value || 'shared-world').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '') || 'shared-world';
}
...
for (const record of manifest.shards || []) {
  ...
  const townKey = record.townKey || shard.townKey || record.key.replace(/-part-\d+$/, '');
  ...
  if ((shard.townKey || townKey) !== townKey) failures.push(`${record.path}: town key does not match manifest.`);
  ...
  if (townKey === 'reference-only') {
    referenceShardSeen = true;
    if (shardCanonical !== 0) failures.push(`${record.path}: reference-only shard contains generation-eligible records.`);
  } else {
    ...
    for (const asset of shard.assets || []) {
      if (slug(asset.town) !== townKey) failures.push(`${record.path}: ${asset.id} belongs to town ${asset.town}, not ${townKey}.`);
    }
  }

  recordsByTown[townKey] = (recordsByTown[townKey] || 0) + (shard.assets?.length || 0);
  for (const asset of shard.assets || []) {
    total += 1;
...
if ((recordsByTown['reference-only'] || 0) !== expectedReferenceOnly) failures.push('Reference-only records are split or missing incorrectly.');
...
console.log(`[sharded-specs] PASS: ${total} records reassembled from ${manifest.shards.length} bounded shards.`);
...
console.log(`[sharded-specs] towns=${Object.entries(recordsByTown).map(([town, count]) => `${town}=${count}`).join(', ')}`);
