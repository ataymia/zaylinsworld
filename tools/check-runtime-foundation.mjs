...
assert.match(state, /SAVE_SCHEMA_VERSION = 8/, 'save schema must include persistent life progression');
...
assert.match(largeTown, /applyHeightFunctionToPlane/, 'large town terrain must use its selected elevation function');
assert.match(largeTown, /buildStarterTownRoadsideLayer/, 'large town must include generated road infrastructure');
assert.match(largeTown, /buildDistrictMassing\(\{ heightAt \}\)/, 'district massing must use the selected terrain elevation');
